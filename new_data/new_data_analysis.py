...
print("{} figures of which {} are valid (1+ valid assessment)".format(len(data.getAllFigures()), len(data.getAllValidFigures())))

print("{} assessments of which {} are valid (18 items)".format(len(data.getAllAssessments()), len(data.getAllValidAssessments()) ))
...
def assessments_per_figure(figures):
    ...
    n, bins, patches = plt.hist(x=number_of_assessments, rwidth=0.85, bins=range(min(number_of_assessments),max(number_of_assessments)+1))
...
def std_scores(figures):
    ...
    plt.ylabel('Frequency')
    plt.title('Histogram of empirical std of scores of {} distinct figures ({}+ assessments)'.format(len(figures), min_number_of_assessments))
    maxfreq = n.max()
    plt.ylim(top=np.ceil(maxfreq / 10) * 10 if maxfreq % 10 else maxfreq + 10)
    plt.show()
...
def std_part_scores(items):
    ...
    plt.xlabel('Empirical standard deviation')
    plt.ylabel('Frequency')
    plt.title('Histogram of STD of item scores of {} item assessments of {} parts'.format(len(items), len(scores_per_part)))
    maxfreq = n.max()
    plt.ylim(top=np.ceil(maxfreq / 10) * 10 if maxfreq % 10 else maxfreq + 10)
    plt.show()
...
def find_outliers(figures):
    ...
    for fig in figures:
        for ass in fig.getValidAssessments():
            ...
            if np.abs(z_score) > 3:
                print("Outlier for figure {}. Score vs. other scores: {}/{}, z-score: {}".format(fig.filename, ass.getScore(), fig.getScores(), z_score))
    ...
    print("There are {} assessments with a fishy zero score far from the other assessments.".format(len(fishy_zero_score)))
...
def assess_quality_of_incomplete_assessments(data):
    ...
    print("For figures with 3+ assessments (including an incomplete one), the average variance among items is {}".format(mean_variance_with_incomplete_assessments))
...
def assess_quality_of_zero_predictions(data):
    ...
    print("There are {} assessments for 5+ figures, of which {} are zero assessments".format(len(all_assessments),len(zero_score_assessments)))
    p = 99
    quantile = np.percentile(all_assessments,p)
    print("The {}% quantile of all assessments is at {}. This corresponds to the {}% quantile ({} assessments) of the zero assessments"
          .format(p, quantile, sum(zero_score_assessments > quantile)*100//len(zero_score_assessments), sum(zero_score_assessments > quantile)))

    plt.hist(x=all_assessments, rwidth=0.85, bins=50, alpha=0.5, weights=np.ones(np.asarray(all_assessments).shape)*1./len(all_assessments))
    plt.hist(x=zero_score_assessments, rwidth=0.85, bins=50, alpha=0.5, weights=np.ones(np.asarray(zero_score_assessments).shape)*1./len(zero_score_assessments))
...
def std_of_scores_vs_label(figures):
    ...
    plt.title('Std of Scores vs label ({} figures with {}+ assessments)'.format(len(figures),min_number_of_assessments))
    plt.show()
...
def difference_median_score(figures):
    ...
    plt.title(
        'Difference Median vs Sum of Item Medians ({} figures with {}+ assessments)'.format(len(figures), min_number_of_assessments))
    plt.show()
    ...
    print("Average difference between median score and sum of item-wise median score: {}".format(np.mean(median_scores_items)-np.mean(median_scores)))
    plt.show()
...
def analyze_persons(data):
    ...
    persons_with_many_assessments = [data.persons[p] for p in data.persons if len(data.persons[p].getValidAssessments()) >= 5]
    cheaters = [data.persons[p] for p in data.persons if data.persons[p].cheater_score is not None and data.persons[p].cheater_score < 0.8]
    print("There are {} person of which {} have scored 1+ valid assessment, {} have scored {}+ assessments, and {} are cheaters by client's definition"
          .format(len(all_persons), len(persons), len(persons_with_many_assessments), min_number_of_assessments, len(cheaters)))
    ...
    figures_cheaters = [fig for fig in data.getAllFigures() if
                        len([ass for ass in fig.assessments if not fig.assessments[ass].invalid and fig.assessments[ass].itemCount()==18])
    ...
    plt.hist(x=mean_diffs, rwidth=0.85, bins=50, alpha=0.5, weights=np.ones(np.asarray(mean_diffs).shape)*1./len(mean_diffs))
    plt.hist(x=mean_diffs_cheaters, rwidth=0.85, bins=50, alpha=0.5, weights=np.ones(np.asarray(mean_diffs_cheaters).shape)*1./len(mean_diffs_cheaters))
    plt.hist(x=mean_diffs_fishy_zero, rwidth=0.85, bins=50, alpha=0.5, weights=np.ones(np.asarray(mean_diffs_fishy_zero).shape)*1./len(mean_diffs_fishy_zero))
    plt.legend(['All persons', 'Cheaters (< 0.8 avgMatch)', 'Persons with at least one fishy zero prediction'],loc='upper left')
...
def distribution_of_human_mse(data, system_MSE):
    persons = [data.persons[p] for p in data.persons if data.persons[p].hasValidAssessment() and (data.persons[p].cheater_score is None or data.persons[p].cheater_score >= 0.8)]
    min_number_of_assessments = 5
    figures = [fig for fig in data.getAllValidFigures() if len(fig.getScores()) >= min_number_of_assessments]
...
def distribution_of_human_accurarcy(data, model_accuracy = None):
    ...
    if model_accuracy is not None:
        ...
        print("With the accuracy of {} the system performs better than {}% of the people".format(model_accuracy, 100*perc))
...
def delete_bad_persons(data):
    ...
    print("{} of {} persons are in the quantile, affecting {} assessments".format(n_bad_persons, len(new_data.persons), n_bad_assessments))
    ...
    print("Before: {} figure, {} valid figures, {} assessments, {} valid assessments, {} persons, {} good persons"
          .format( len(figures), len(valid_figures), len(assessments), len(valid_assessments), len(persons), len(good_persons) ))
...
def score_distribution_adults_vs_children(figures):
    ...
    plt.hist(x=adults, rwidth=0.85, bins=np.linspace(0,36,37), alpha=0.5, label="Adults", weights=np.ones(np.asarray(adults).shape) * 1. / len(adults))
    plt.hist(x=children, rwidth=0.85, bins=np.linspace(0,36,37), alpha=0.5, label="Children", weights=np.ones(np.asarray(children).shape) * 1. / len(children))
...
def std_adults_vs_children(figures):
    ...
    plt.ylabel('Fraction')
    plt.title('Histogram of standard deviations of scores of adults ({}) vs children ({})'.format(len(adults), len(children)))
    plt.show()
