...
def visualize_labels(dataset):
    ...
    plt.title('Histogram of labels of {} images of the data set {}'.format(dataset.labels.shape, dataset.name))
    maxfreq = n.max()
...
def visualize_mean_image(dataset):
    ...
    plt.title('Mean image of the data set {}'.format(dataset.name))
...
def save_image(img, filename, border=False):
    ...
    imsave("../data/visualized/preprocessing/"+filename,img)
...
def compare_two_images(img1,img2, dir="../data/visualized/comparison/", filename="image.jpg"):
