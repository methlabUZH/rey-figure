...
from augmenter import augment, simulate_augment
from helpers import imgs_to_file
...
def preprocess_images(images, set="train"):
    if(DEBUG):
...
if REDO_PREPROCESSING_IMAGES:
    ...
    if not DATA_AUGMENTATION:
        ...
        if CONVERGENCE == 0:
            ...
            DATA = Dataset("DATA", np.asarray(images)[0:CONVERGENCE, :], files[0:CONVERGENCE], labels[0:CONVERGENCE])
        ...
        while batch_size*l < len(figures):
            ...
            images = [fig.getImage() for fig in figures[l*batch_size:np.min([(l+1)*batch_size, len(figures)])]]
            ...
            end_preprocessing = time.perf_counter()
            if DEBUG:
                print("preprocessing of {} images took {}s".format(len(images), end_preprocessing - start_preprocessing))
