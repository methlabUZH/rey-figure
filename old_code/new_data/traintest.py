...
test_dir = "Data_test/"
all_dir = "uploadFinal/"
check_folder = "../preprocessing/raw/"
...
if np.shape(tobesplit_indices)[0] > 0:
    test_size = ((np.shape(test_indices)[0] + np.shape(train_indices)[0] + np.shape(tobesplit_indices)[0]) * test_size -
                 np.shape(test_indices)[0]) / np.shape(tobesplit_indices)[0]
