...
if VISUALIZE_AUGMENTATION:
    create_directory("../preprocessing/visualized/augment", empty_dir=True)
...
def augment(image, alpha=None, sigma=5, max_factor=None, degrees=None):
    """Augment: function to augment input preprocessing using various procedures
    ...
    if VISUALIZE_AUGMENTATION:
        ...
        imgs_to_file([orig,step_elastic,step_skew,step_rotate,step_cutdown,image,np.zeros((116,10)),blurred],"../preprocessing/visualized/augment/"+str(time.time())+".jpg")
...
def simulate_augment(image):
    """
    augmented preprocessing is different from original, e.g. a bit blurred
