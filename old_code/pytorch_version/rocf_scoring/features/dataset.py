...
class ROCFDataset(Dataset):
    ...
    def __init__(self, figures, labels, files, csv_preprocessed=None, csv_raw=None):
        """
        Args:
            preprocessing (): Path to the csv file with annotations.
            csv_preprocessed (string, optional): csv file containing information about preprocessed preprocessing
                                                 (paths, labels, ...)
            csv_raw (string, optional): csv file containing information about raw preprocessing (paths, labels, ...)
        """
...
if __name__=="__main__":
    ...
    for (samples, _labels, one_hot_labels) in dataloader:
        ...
        print("preprocessing: ", samples.size())
