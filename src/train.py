...
def train_cnn(run_name, fold, data_train, data_val):
    ...
    if(DEBUG):
        if TEST:
            print("\n\nStart training model on all training data. Run: {}".format(run_name, fold))
        else:
...
def cross_validate(data, k=10, skip=[]):
    ...
    if not os.path.exists('results.csv'):
        with open('results.csv', 'w') as f:
            writer = csv.writer(f)
            fields = ["Time", "Run name", "data augmentation", "label format", "binning", "conv layers", 'mean', 'val results']
...
def test():
    ...
    with open(result_file_path, "a") as result_file:
        result_file.write("\nRun parameters:\n {}\n".format(RUN_PARAMETERS))
        result_file.write("\nRun: {}, train on all training data, test on test".format(run_name))
        result_file.write("\nTest Results\n")
