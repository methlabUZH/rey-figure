...
def make_plot(results_dir, pmeasure=ABSOLUTE_ERROR, save_as=None):
    ...
    bin_metrics = pm.bin_level_metrics_report(bin_granularity=4)
    ...
    plt.errorbar(range(len(x_labels)), y_values, yerr=y_errs.T, elinewidth=1.0, capsize=5, capthick=2,
                 ls='-.', marker='d', label='CNN with Data Augmentation', color=colors[1])
