...
# load only few images per data set, not all (to speed up testing)
# LOAD_ONLY_FEW = False
# Read in via config file

# preprocessed images (incl. labels) are written to disk for faster recovery
# if raw data or preprocessing changes, you have to redo it by setting this to True
...
class Dataset:
    ...
    def visualize_dataset(self, n = 50, directory = DATA_DIR + "visualized/"):
        """
        Visualizes data set by writing original image (from path) vs image in data set to disk
...
# check if preprocessed data available -> don't redo anything
if REDO_PREPROCESSING or not os.path.exists(DATA_DIR + "serialized/images.npy"):
    ...
    # reload preprocessed data from disk
    if(DEBUG):
