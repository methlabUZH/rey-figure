...
class ReyClassifier2(nn.Module):
    """
    ...
    def __init__(self,
        ...
        super(ReyClassifier2, self).__init__()
...
class ReyClassifier(nn.Module):
    ...
    def __init__(self,
        ...
        self._block3 = conv_block([128, 256], [256, 256], [1, 1], (3, 3), norm_layer_2d, None, None, dropout_rates[0],
                                  pooling_layer=nn.AdaptiveMaxPool2d(output_size=(1, 1)))

        # fc layers
        self._dense_layer1 = fc_layer(256, 512, norm_layer=norm_layer_1d, dropout_rate=dropout_rates[1])
        self._dense_layer2 = fc_layer(512, 512, norm_layer=norm_layer_1d, dropout_rate=dropout_rates[1])

        self._output_layer = nn.Sequential(nn.Linear(in_features=512, out_features=2),
                                           nn.Softmax(dim=1))
    ...
    def forward(self, inputs):
        ...
        prediction = self._output_layer(x)
...
def get_reyclassifier(dropout: Tuple[float, float],
    ...
    return ReyClassifier2(dropout_rates=dropout, norm_layer_2d=norm_layer_2d, norm_layer_1d=norm_layer_1d)
